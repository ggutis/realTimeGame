...
import { DamageEvent } from '../../generated/game';
import { GameSession } from '../../models/game.session';


export function handleAnimalLogic(session: GameSession){
			...
			if (minDistanceToMonster <= animalData.range) {
				...
				if (animal.attackSpeed <= 0) {
					...
					session.damageEvents.push({
						...
						isMonster: true, // 몬스터가 피해를 입었으므로 true
					});
				}
			} else if (
				nearestFriendlyAnimal?.type === animal.type &&
				minDistanceToFriendly < UNIT_BUFFER_DISTANCE
			) {
				animal.isMoving = false;
			} else {
