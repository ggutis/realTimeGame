...
export function handleMonsterLogic(session: GameSession) {
	...
	for (const monster of Object.values(session.activeMonsters)) {
			...
			if (distance <= monsterData.range) {
				if (monster.attackSpeed <= 0) {
					...
					session.damageEvents.push({
						...
						isMonster: false, // 유닛이 피해를 입었으므로 false
					});
				}
			}
		} else {
		...
		session.damageEvents.push({
			targetId: 'base', // 기지임을 나타내는 ID 사용
			damage: monsterData.damage,
			isMonster: false, // 기지는 몬스터가 아니므로 false
		});
